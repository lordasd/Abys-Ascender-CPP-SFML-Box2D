...
SkeletonMelee::SkeletonMelee(const std::vector<sf::Sprite>& animation, float interval, const sf::Vector2f& position, float velocity, b2World* world)
    :   Mob(animation, interval, velocity, world, SoundManager::instance().getSwordWooshPlayer(), WeaponType::Sword, SKELETON_YOFFSET_POS, SKELETON_DAMAGE, SKELE_MELEE_ATTACK_COOLDOWN, SKELETON_HEALTH),
        ...
        m_hurt(SoundManager::instance().getSkeletonDeath())
{
    ...
    m_mobState = std::make_unique<RoamingState>(SKELETON_DIR_INTERVAL,
        [&](Mob& mob) {
            mob.setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Stay)), IDLE2);
        },
        [&](Mob& mob) {
            mob.setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Run)), WALK);
        });
...
void SkeletonMelee::update(float deltaTime)
{
    ...
    {   // Different states (change per distance)
        if (m_stateID == MobStateID::Roam)
        {
            if (std::abs(m_playersPosition.y - skeletonPos.y) <= 22.f &&
                std::abs(m_playersPosition.x - skeletonPos.x) <= 75.f &&
                std::abs(m_playersPosition.x - skeletonPos.x) >= 30.f)
            {
                ...
                std::abs(m_playersPosition.x - skeletonPos.x) < 35.f)
            {
                m_mobState = std::make_unique<AttackingState>(SKELE_MELEE_ATTACK_COOLDOWN,
                    [&](Mob& mob) {
                        mob.setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Stay)), IDLE2);
                    },
                    [&](Mob& mob) {
                        mob.setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Attack)), SKELE_ATTACK);
                    });
                m_stateID = MobStateID::Attack;
                setOperation(Operation::Stay);
                setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Stay)), IDLE2);
            }
        }
        ...
        {
            if (std::abs(m_playersPosition.y - skeletonPos.y) > 22.f &&
                std::abs(m_playersPosition.x - skeletonPos.x) > 37.f)
            {
                m_mobState = std::make_unique<RoamingState>(SKELETON_DIR_INTERVAL,
                    [&](Mob& mob) {
                        mob.setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Stay)), IDLE2);
    ...
    else // Skeleton is dead
    {
        ...
        setAnimation(TextureManager::instance().getSkeletonMeleeAnimation(getAnimation().animationID(AnimationType::Die)), SKELE_DIE);
